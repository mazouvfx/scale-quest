...
import { getAllScales, NOTES, buildScale, SCALE_PATTERNS } from '@/lib/scales'
import { LICKS, type Lick } from '@/lib/licks'
import { useMidi, midiToName } from '@/lib/midi'
...
const FAMILIES = [
  ...
  { value: 'diminished', label: 'Diminuée' },
...
const BADGE_DEFS = [
  ...
  { slug: 'midi-play', label: '🎹 MIDI connecté', cond: (s: GameState) => s.midiUsed },
]
interface GameState {
...
export default function Home() {
  ...
  const [state, setState] = useState<GameState>({ score:0, combo:0, xp:0, level:1, badges: new Set(), midiUsed:false })
  ...
  const onMidiNote = useCallback((midi: { note: number; name: string; velocity: number }) => {
    ...
    if (midiExpected.length > 0) {
      const expected = midiExpected[midiProgress]
      if (expected && midi.name === expected) {
        const next = midiProgress + 1
        setMidiProgress(next)
        if (next >= midiExpected.length) { handleCorrect(); setMidiProgress(0); setMidiExpected([]) }
      }
    }
  }, [midiExpected, midiProgress])
  ...
  function handleWrong() { setState(prev => ({ ...prev, combo: 0 })) }
  ...
  function startRoll(notes: string[]) {
    buildRollNotes(notes); setRolling(true); rollOffsetRef.current = 0
    const pxPerBeat = 900 / (tempo / 60 * 4)
    const spd = pxPerBeat / 60
    const step = () => {
      ...
      else { setRolling(false); setActiveKeys([]) }
    }
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        <header className="flex items-center justify-between mb-6 flex-wrap gap-4">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-neon-cyan to-neon-pink grid place-items-center shadow-lg">
              <span className="text-xl">🎹</span>
            </div>
            <div>
              <h1 className="font-display text-3xl font-semibold tracking-tight">Scale Quest</h1>
              <p className="text-slate-400 text-sm">Mémorise gammes, licks et patterns</p>
            </div>
          </div>
          <div className="flex items-center gap-3 flex-wrap">
            {connected ? (
              <span className="chip border-neon-cyan/40 text-neon-cyan">🎹 {deviceName ?? 'MIDI connecté'}</span>
            ) : (
              <span className="chip text-slate-500">🔌 Aucun MIDI</span>
            )}
            <span className="chip">Score {state.score}</span>
            <span className="chip border-neon-gold/40 text-neon-gold">XP {state.xp}</span>
          </div>
        </header>

        {/* XP BAR */}
        <div className="mb-6 flex items-center gap-3">
          <span className="text-xs text-slate-500">Niv. {state.level}</span>
          ...
          <span className="text-xs text-slate-500">{state.xp} XP</span>
        </div>

        {/* TABS */}
        <div className="flex gap-2 mb-5">
          {(['quiz','roll','licks'] as const).map(t => (
            <button key={t} className={`btn text-sm capitalize ${tab===t ? 'btn-primary' : ''}`} onClick={()=>setTab(t)}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          <div className="lg:col-span-2 flex flex-col gap-5">
            {tab === 'quiz' && (
              <section className="panel">
                ...
                {question ? (
                  <>
                    <div className="rounded-2xl bg-panel-2 border border-panel-border p-4 mb-4 text-base font-medium">{question.text}</div>
                    ...
                    {chosen && (
                      <div className={`mt-4 rounded-2xl p-3 text-sm ${chosen===question.correct ? 'bg-neon-green/10 border border-neon-green/30 text-neon-green' : 'bg-neon-red/10 border border-neon-red/30 text-neon-red'}`}>
                        {chosen===question.correct ? '✅ Correct' : '❌ Raté — bonne réponse : '+question.correct}
                    ...
                    {connected && midiExpected.length > 0 && (
                      <div className="mt-3 rounded-2xl bg-panel-2 border border-neon-cyan/20 p-3 text-sm">
                        ...
                        {midiExpected.map((n,i) => (
                          <span key={i} className={`inline-block mr-1 px-2 py-0.5 rounded-full text-xs font-bold ${i < midiProgress ? 'bg-neon-green/20 text-neon-green' : i===midiProgress ? 'bg-neon-cyan/20 text-neon-cyan animate-pulse' : 'bg-panel border border-panel-border'}`}>{n}</span>
                        ))}
                      </div>
                    )}
                  </>
            ...
            {tab === 'roll' && (
              <section className="panel">
                ...
                <div className="relative rounded-2xl overflow-hidden border border-panel-border bg-[#080c14]" style={{height:340}}>
                  {rollNotes.map(n => (
                    <div key={n.id} className="absolute rounded-xl transition-none" style={{left:n.x,top:n.y,width:n.w,height:n.h,background:`linear-gradient(180deg,${n.color},${n.color}88)`,boxShadow:`0 0 10px ${n.color}66`}}>
            ...
            {tab === 'licks' && (
              <section className="panel">
                ...
                <div className="rounded-2xl bg-panel-2 border border-panel-border p-4">
                  ...
                  <div className="bg-neon-gold/5 border border-neon-gold/20 rounded-xl p-3 text-sm text-neon-gold/90">{lick.tip}</div>
                  <div className="flex gap-3 mt-4">
                    <button className="btn btn-primary" onClick={()=>startRoll(lick.notes)}>Lire le lick</button>
                    <button className="btn" onClick={()=>{ setActiveKeys(lick.notes); setTimeout(()=>setActiveKeys([]),1500) }}>Voir sur le clavier</button>
                  </div>
                </div>
                <PianoKeys active={activeKeys}/>
              </section>
          ...
          <div className="flex flex-col gap-5">
            <section className="panel">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="stat-card"><div className="text-slate-500 text-xs">Score</div><div className="text-2xl font-display font-semibold text-neon-cyan">{state.score}</div></div>
                <div className="stat-card"><div className="text-slate-500 text-xs">Combo</div><div className="text-2xl font-display font-semibold text-neon-pink">{state.combo}x</div></div>
                <div className="stat-card"><div className="text-slate-500 text-xs">Niveau</div><div className="text-2xl font-display font-semibold text-neon-gold">{state.level}</div></div>
                <div className="stat-card"><div className="text-slate-500 text-xs">XP</div><div className="text-2xl font-display font-semibold text-neon-purple">{state.xp}</div></div>
              </div>
            </section>
            <section className="panel">
              ...
              <div className="flex flex-wrap gap-2">
                {BADGE_DEFS.map(b => (
                  <span key={b.slug} className={`badge ${state.badges.has(b.slug) ? 'badge-earned' : 'opacity-40'}`}>{b.label}</span>
                ))}
              </div>
            </section>
            <section className="panel">
              <h2 className="font-display text-lg font-semibold mb-2">Setup MIDI</h2>
              <p className="text-slate-400 text-sm mb-3">Connecte ton Roland Juno D7 via USB pour jouer directement dans l'app.</p>
              <div className={`rounded-xl p-3 text-sm border ${connected ? 'border-neon-cyan/30 bg-neon-cyan/5 text-neon-cyan' : 'border-panel-border bg-panel-2 text-slate-500'}`}>
                {connected ? `✅ ${deviceName}` : '🔌 Aucun appareil MIDI détecté'}
              </div>
              <p className="text-slate-500 text-xs mt-2">Sur iPad, utilise l'app <strong className="text-slate-400">Web MIDI Browser</strong>.</p>
            </section>
          </div>
        </div>
      </div>
    ...
    <div className="grid mt-4 gap-0.5" style={{gridTemplateColumns:`repeat(${whites.length},1fr)`}}>
      {whites.map((n,i) => (
        <div key={i} className={`key-white h-20 ${active.includes(n) ? 'key-active' : ''}`}>
          {i===0||i===7?<span className="text-[9px]">{n}</span>:null}
        </div>
      ))}
    </div>
  )
